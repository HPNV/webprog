...
@section('content')
    <div class="relative bg-gradient-to-r from-blue-700 to-blue-900 h-96">
        <img src="{{ $place->imagePath }}" alt="{{ $place->name }}" class="w-full h-full object-cover opacity-60">
        <div class="absolute inset-0 flex flex-col justify-center items-center text-white">
            <h1 class="text-5xl font-extrabold shadow-md">{{ $place->name }}</h1>
            <p class="text-xl mt-2 italic">{{ $place->tagline }}</p>
        </div>
    </div>
